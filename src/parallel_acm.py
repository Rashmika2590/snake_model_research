...
def bilinear_interpolate(image, x, y):
    ...
    if x0 < 0 or x1 >= w or y0 < 0 or y1 >= h:
        return 0.0  # Return 0 for points outside the image
...
def _parallel_snake_iteration(snake, image_energy, search_window, alpha, beta, w_edge):
    ...
    # Manually roll the snake array to avoid Numba compatibility issues with np.roll's axis argument
    rolled_snake = np.empty_like(snake)
    rolled_snake[:-1] = snake[1:]
    rolled_snake[-1] = snake[0]
    # Manual norm calculation because Numba doesn't support axis argument in np.linalg.norm
...
def run_parallel(image, initial_snake):
    ...
    # Pre-computation (done in Python/NumPy)
    ...
    # The main loop calls the Numba-jitted function
